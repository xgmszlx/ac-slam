...
MyPlanner::MyPlanner()
{
	...
	this->mActiveLoopVertex = -1;
	this->mLoopExecutionStartLogged = false;
...
void MyPlanner::setReliableLoopPath(const int& currGoal){
	...
	this->mActiveLoopVertex = currGoal;
	this->mLoopExecutionStartLogged = false;
	std::stringstream loopPathStream;
	loopPathStream << std::setprecision(9);
	for(size_t i = 0; i < this->mClosingPath.size(); i++){
		if(i > 0){
			loopPathStream << ";";
		}
		loopPathStream << this->mClosingPath[i].first << "," << this->mClosingPath[i].second;
	}
	ROS_INFO("PHASE2_LOOP_PLANNED vertex=%d source=%s waypoints=%zu path=%s",
			 currGoal, getReliableLoop ? "reliable_loop_service" : "fallback",
			 this->mClosingPath.size(), loopPathStream.str().c_str());
	this->mCurrClosingIdx = 0;
	return;
}
...
bool MyPlanner::performReliableLooping(GridMap* map, unsigned int start, unsigned int &goal){
	...
	if(this->loopVertexReached(map, start)){
		ROS_INFO("PHASE2_LOOP_WAYPOINT_REACHED vertex=%d waypoint=%d",
				 this->mActiveLoopVertex, this->mCurrClosingIdx);
...
{	
	...
	*/
	...
	if(this->mLoopClosing){  // Finish reliable loop closing
		ROS_INFO("PHASE2_LOOP_EXECUTION_FINISHED vertex=%d waypoints=%zu",
				 this->mActiveLoopVertex, this->mClosingPath.size());
