...
class PathPlanner:
    def __init__(self):
        ...
        self.tsp_seed = int(rospy.get_param('/path_planner/tsp_seed', -1))
        self.tsp_record_path = rospy.get_param(
            '~tsp_record_path',
            os.path.join(os.path.dirname(os.path.realpath(__file__)), 'tsp_record.json')
        )
        ...
        self.prior_map_save_path = rospy.get_param(
            '~prior_map_save_path',
            os.path.join(os.path.dirname(os.path.realpath(__file__)), 'prior_map.pickle')
        )
        prior_map_save_dir = os.path.dirname(os.path.abspath(self.prior_map_save_path))
        os.makedirs(prior_map_save_dir, exist_ok=True)
        save_data(self.prior_graph, self.prior_map_save_path)
        rospy.loginfo('prior_map_save_path: {}'.format(self.prior_map_save_path))
    ...
    def solve_tsp_path(self):
        ...
        if self.tsp_solver_name == "tsp-solver":
            ...
            predicted_tsp_length = sum(D[idx_path[i]][idx_path[i + 1]] for i in range(len(idx_path) - 1))
            ...
            time1 = time.time()
            concorde_seed = self.tsp_seed if self.tsp_seed >= 0 else None
            new_tsp_path, new_tsp_distance = concorde_tsp_solver(
                distance_matrix, concorde_node_list, seed=concorde_seed
            )
            tsp_solve_time = time.time() - time1
            rospy.logwarn(f"Tsp problem solved by Concorde. {tsp_solve_time}")
            self.tsp_path = new_tsp_path
            predicted_tsp_length = new_tsp_distance
        ...
        record = {
            "solver": self.tsp_solver_name,
            "seed": self.tsp_seed if self.tsp_solver_name == "concorde" and self.tsp_seed >= 0 else None,
            "initial_tsp_path": [int(node) for node in self.tsp_path],
            "full_tsp_path": [int(node) for node in self.full_tsp_path],
            "predicted_tsp_length": float(predicted_tsp_length),
            "predicted_full_tsp_length": float(get_path_length(
                self.prior_graph, self.full_tsp_path, weight="weight"
            ))
        }
        tsp_record_dir = os.path.dirname(os.path.abspath(self.tsp_record_path))
        os.makedirs(tsp_record_dir, exist_ok=True)
        with open(self.tsp_record_path, 'w') as tsp_record_file:
            json.dump(record, tsp_record_file, indent=2, sort_keys=True)
            tsp_record_file.write('\n')
        rospy.loginfo('TSP_RECORD {}'.format(json.dumps(record, sort_keys=True)))
        rospy.loginfo('tsp_record_path: {}'.format(self.tsp_record_path))
        return
