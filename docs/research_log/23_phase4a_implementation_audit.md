...
| `handle_reliable_loop` `oracle_mode=2`: redesigned gate G1′ (V0 history span < 4.0 m = `LoopMatchMinimumChainSize` 4 × `MinimumTravelDistance` 1.0, **fixed Karto config**) → REPAIR; else NO_REPAIR returns the original V0 path. Bounded repair = contiguous pose-graph segment (≤ 12 m / ≤ 24 wp, 0.5 m densify, forward/reverse by entry-distance + heading alignment). Yaw used only as direction cue. | `scripts/path_planner.py` | baseline `7c20c3c` |
